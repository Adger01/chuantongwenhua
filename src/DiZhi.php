...
class DiZhi
{
     ...
     * 根据地支获取其对应的五行
     */
    public static function getWuxingByZhi($zhi)
    {
        return self::$zhiWuxing[$zhi] ?? "";
    }
     ...
     * 根据地支和月支获取其五行强度
     */
    public static function getWeightByZhiAndMonth($zhi, $zhi_month)
    {
        $ganggan = self::$weight[$zhi];
        $data = [];
        foreach ($ganggan as $gan => $item) {
            $data[$gan] = $item[$zhi_month];
        }
        return $data;
    }
}
